...
package com.airlife.control.env;
...
public class ImageUtils {
  ...
  public static void convertRGBAToARGB8888(
          byte[] input,
          int width,
          int height,
          int[] output) {
    if (useNativeConversion) {
      try {
        ImageUtils.convertRGBAToARGB8888(input, output, width, height);
        return;
      } catch (UnsatisfiedLinkError e) {
        LOGGER.w(
                "Native RGBA -> RGBA8888 implementation not found, falling back to Java implementation");
        useNativeConversion = false;
      }
    }
    ...
    for(int i=0;i<frameSize;i++){
      output[i] = input[i*4+3]<<24 | input[i*4]<<16 | input[i*4+1]<<8 | input [i*4+2];
    }
  }

  public static void convertRGBAToLUMINANCE(
          byte[] input,
          int width,
          int height,
          byte[] output) {
    if (useNativeConversion) {
      try {
        ImageUtils.convertRGBAToLUMINANCE(input, output, width, height);
        return;
      } catch (UnsatisfiedLinkError e) {
        LOGGER.w(
                "Native RGBA -> Luminance implementation not found, falling back to Java implementation");
        useNativeConversion = false;
      }
    }
    ...
    final int frameSize = width * height;
    for(int i=0;i<frameSize;i++){
      output[i] = (byte)(input[i*4]*0.299 + input[i*4+1]*0.587 + input [i*4+2]*0.114);
    }
  }
  ...
  /**
   * Converts RGBA semi-planar data to ARGB 8888 data using the supplied width and height. The
   * input and output must already be allocated and non-null. For efficiency, no error checking is
   * performed.
   *
   * @param input The array of RBA
   * @param output A pre-allocated array for the ARGB 8:8:8:8 output data.
   * @param width The width of the input image.
   * @param height The height of the input image.
   */
  private static native void convertRGBAToARGB8888(
   ...
   *
   * @param input The array of RBA
   * @param output A pre-allocated array for Luminance output data.
   * @param width The width of the input image.
   * @param height The height of the input image.
   */
  private static native void convertRGBAToLUMINANCE(
          byte[] input, byte[] output, int width, int height);
